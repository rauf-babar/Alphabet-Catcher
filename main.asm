[org 0x0100]
	...
	CharColumns:  times 10 dw -1          ; array of Columns
	CharOffsets:  times 10 dw -1          ; Array for offsets initialized to -1 (indicates available slots)
	Characters:   times 10 dw -1          ; Array for characters initialized to -1 (indicates available slots)
	
	currentFrame:  dw 1       ; frame rate for speed of Characters
	...
	SystemSpeed:   dw 4       ; speed of tisr to execute (half Second is normal)
	timeTick:      dw 0       ; timer counter for time period
	...
	ResetData:                       ; Subroutine to reset data for restart game
		mov word [numberOfAlphabets], 6
		mov word [terminateLoops], 12
		mov word [rand], 0
		mov word [randnum], 0
		mov word [Options], 0
		mov word [Esc_Key], 0
		...
		mov word [currentFrame], 1
		mov word [SystemSpeed], 4
		mov word [timeTick], 0
		mov word [timeMin], 0
		mov word [timeSec], 0
	...
	kbisr:              ; keyboard interrupt service routine
		...
		shiftCall:
		mov cx, [speedVal]                   ; no of box movements
	...
	Multiplayer_Setup:            ; Subroutine to setup Multiplayer game
		pusha
	...
	Check_Offsets:      ; Subroutine to modify offset on basis of location
		...
		check_eachOffset:
			...
			jb skip_Offset_Reset
				...
				addToScore:
					...
					jbe dontIncrement2
						mov word [SystemSpeed], 2  ; increase speed of system tisr execution
					dontIncrement2:
		...
		returnToPlayAgain:
			mov word [bp + 4], 1          ; return 1 incase of exit
	...
	printOffset:
	 ...
	 StartGame:
		...
		inc word [timeTick]	
		mov ax, [timeTick]              ; store current time tick
		cmp word [timeTick], 18      	; tick count for second (approx)
		jne SkipSec_Inc	
			mov word [timeTick], 0   	; reset time tick count
			inc word [timeSec]       	; increment Second
			cmp word [timeSec], 60      ; if second equal to 60
			jne SkipSec_Inc
				mov word [timeSec], 0   ; reset secind
				inc word [timeMin]      ; increment min
		
		SkipSec_Inc:
		call GenerateData            ; Generate Random Data
		...
		mov si, 0                    ; index 0 for moving along array
		mov di, 1                    ; divider (index of array)
		incrementRows:
			mov ax, [currentFrame]
			mov bx, di
			div bl
		...
		cmp ax, 1
		jne IncreaseFrameRate
			mov word [bp + 2], PlayAgain
			mov ax, [CS_Segment]
			mov word [bp + 4], ax
		
		IncreaseFrameRate:
		inc word [currentFrame]       ; reset frame rate
		cmp word [currentFrame], 49
		jne exit_StartGame
			mov word [currentFrame], 1
	...
	mov [es:9*4+2], cs
	mov word [es:8*4], StartGame          ; Hook timer interrupt
	mov [es:8*4+2], cs
	sti
	
	GameLoop: jmp GameLoop                ; infinte game loop
	...
	endOfGame:                            ; else exit and restore cmd screen
	call restoreScreen
